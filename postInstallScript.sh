...
sudo apt-get update -y && sudo apt upgrade -y && sudo apt install -y python3-pynvim sddm flatpak fzf dolphin firefox-esr wmname rustdesk syncthing code ripgrep fonts-hack-ttf brasero rofi feh polybar tree chromium cmake gnome-disk-utility netcat-openbsd gparted kate keepassxc btop prusa-slicer maim fastfetch nmap p7zip-full python3-pip ripgrep sxiv syncthing terminator ufw unzip vim $virtualbox_name wget whois xclip zenity zsh
